...
const register = async (req, res) => {
    try {
        const { name, email, phone, address, city, state, country, pincode, role, password } = req.body;
        ...
        otpStore[email] = {
            otp,
            userData: { name, email, phone, address, city, state, country, pincode, role, password: hashedPassword },
...
const verifyOtp = async (req, res) => {
    try {
        ...
        // Create user
        const user = new userModel(record.userData);
        await user.save();
        delete otpStore[email];
        // Generate JWT token
